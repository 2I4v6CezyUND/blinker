...
RSpec.describe ModelsController, type: :controller do
  ...
  let(:audi) { FactoryBot.create(:make, name: "Audi")}
  let(:valid_attributes) {
    FactoryBot.build(:model, name: "A4", make: audi).attributes
  }
  ...
  describe "PUT #update" do
    context "with valid params" do
      ...
      it "updates the requested model" do
        ...
        model.reload
	expect(JSON.parse(response.body)).to eql(JSON.parse(model.to_json))
      end
    ...
    context "with invalid params" do
      it "renders a JSON response with errors for the model" do
        ...
        put :update, params: {id: model.to_param, model: {name: nil}}, session: valid_session
        expect(response).to have_http_status(:unprocessable_entity)
        expect(response.content_type).to eq('application/json')
      end
    end
